...
import com.jflyfox.modules.admin.recruitingtrip.TbRecruitingTrip;
...
public class FrontCacheService extends BaseService {
	...
	public List<TbFolderRollPicture> getRollPicture(int folderId) {
		...
		String sql = "select * from tb_folder_roll_picture t " //
				...
				+ " and folder_id = ? and identify='' order by sort,create_time desc";
		List<TbFolderRollPicture> list = TbFolderRollPicture.dao.findCache(cacheName, key, sql, folderId);
		return list;
	}

	public List<TbFolderRollPicture> getRollPicture(int folderId,int position) {
		String key = ("getRollPicture_" +folderId+ position);
		String sql = "select * from tb_folder_roll_picture t " //
				+ " where is_deleted = " + JFlyFoxUtils.IS_DELETED_NO //
				+ " and folder_id = ? and position = ?  order by sort,create_time desc";
		List<TbFolderRollPicture> list = TbFolderRollPicture.dao.findCache(cacheName, key, sql, folderId,position);
		return list;
	}
	 ...
	 * @return
	 */
	public List<TbRecruitingTrip> getRecruitingtrip(int folderId) {
		String key = ("getRecruitingtrip_" + folderId);
		String sql = "select * from recruiting_trip t " //
				+ " where is_deleted = " + JFlyFoxUtils.IS_DELETED_NO  //
				+ " and folder_id = ? and status=1  order by sort,create_time desc";
		List<TbRecruitingTrip> list = TbRecruitingTrip.dao.findCache(cacheName, key, sql, folderId);
		return list;
	}
	public TbFolderRollPicture returnImageorText(String identify,int folderId){
		String key = ("returnImage_" +identify+folderId);
		String sql = "select * from tb_folder_roll_picture t " //
				+ " where is_deleted = " + JFlyFoxUtils.IS_DELETED_NO //
				+ " and folder_id = ? and identify = ? ";
		List<TbFolderRollPicture> list = TbFolderRollPicture.dao.findCache(cacheName, key, sql, folderId,identify);
		return list.get(0);
	}
}
